...
초안 개수와 카테고리 풀은 `routine.json` 하나에서 관리합니다.
...
- `categories` 에는 **네이버 블로그에 실제로 존재하는** 카테고리만 넣으세요.
  `auto_post.py` 는 `대분류 > 중분류` 의 마지막 항목으로 드롭다운을 클릭하므로,
  없는 이름이면 카테고리 선택이 실패하고 기본 카테고리로 발행됩니다.
- 카테고리는 **하루 안에서 중복 없이** 1글 1카테고리로 배정됩니다.
  따라서 하루 6개를 뽑으려면 카테고리가 **6개 이상** 있어야 합니다.
  부족하면 루틴이 가능한 개수만 만들고 몇 개가 더 필요한지 보고합니다.
